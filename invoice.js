...
$( document ).ready( () => {
    ...
    $("#calculate").click( () => {
        ...
        if(invoiceDate != "" && !datePattern.test(invoiceDate)){
            alert("Date must be valid and in MM/DD/YYYY format.");
            $("#clear").click();
            $("#invoice_date").focus();
        }
        let invDate;
        if(invoiceDate == ""){
            ...
            let yy = invoiceDate.slice(6,10);
            let mm = parseInt(invoiceDate.slice(0,2)) - 1;
            let dd = invoiceDate.slice(3,5);
            invDate = new Date(yy, mm, dd); 
        }
        let inDate = dateFormat(invDate);
        invDate.setDate(invDate.getDate() + 30);
        let dueDate = dateFormat(invDate);
        $("#invoice_date").val( inDate );
